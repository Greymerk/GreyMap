...
from nbt.block_entity import BlockEntity
from nbt.block_entity import Sign
...
class Chunk(object):
    ...
    def __init__(self, nbt):
        self.version = nbt['DataVersion'].value
...
class AnvilSection(object):
    ...
    def __init__(self, nbt, version):
        ...
        elif version >= 2566: # MC 1.16.0 to MC 1.17.2 (latest tested version)
    ...
    def _init_index_unpadded(self, nbt):
        ...
        if(self.version >= 2860):
            for p in nbt['palette']:
                name = p['Name'].value
                self.names.append(name)

            states = nbt['block_states'].value
        else:
            for p in nbt['Palette']:
                name = p['Name'].value
                self.names.append(name)
    ...
    def _init_index_padded(self, nbt):
        ...
        if self.version >= 2860:
            if not 'data' in nbt['block_states']:
                states = [0]*256
            else: 
...
class AnvilChunk(Chunk):
    ...
    def __init__(self, nbt):
        ...
        try:
            ...
            if version != 1343 and not (version >= 1631 or version <= 2860):
    ...
    def get_last_update(self):
        if not 'LastUpdate' in self.chunk_data:
            return 0
            
        return self.chunk_data['LastUpdate'].value
    ...
    def get_signs(self):
        ...
        if (not (BLOCK_ENTITIES in self.chunk_data)):
            return []
        block_entities = self.chunk_data[BLOCK_ENTITIES]
        signs = []
        for i, nbt in enumerate(block_entities):
            be = BlockEntity(nbt)
            if(be.getBlockName() == 'minecraft:sign'):
                signs.append(Sign(nbt))
        return signs
